...
import sys
sys.path.insert(0, '%s/../third_party'%os.path.join(os.path.dirname(__file__)))
import numpy as np
...
class BaseDataset(Dataset):
    ...
    def __init__(self, opts, filter_key=None):
        ...
        self.flip=0
        self.crop_factor = opts['crop_factor']
        self.load_pair = True
    ...
    def read_raw(self, im0idx, flowfw,dframe):
        ...
        try:
            ...
            rtk = np.zeros((4,4))
            rtk[:3,:3] = np.eye(3)
            rtk[:3, 3] = np.asarray([0,0,3])
            rtk[3, :]  = np.asarray([-1,-1,-1,-1]) 

        # create mask for visible vs unkonwn
        if mask.min()<0:
            # set to invisible is detection failed
        ...
        nml = cv2.remap(nml,x0,y0,interpolation=cv2.INTER_LINEAR)
        ...
        nml = np.transpose(nml, (2, 0, 1))
        mask = (mask>0).astype(float)
        
        #TODO transform dp feat to same size as img
        dp_feat_rsmp,_ = resample_dp(F.normalize(torch.Tensor(dp_feat)[None],2,1),
