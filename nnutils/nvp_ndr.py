...
class DeformNetwork(nn.Module):
    ...
    def forward(self, deformation_code, input_pts, alpha_ratio):
        ...
        for i_b in range(self.n_blocks):
            ...
            rot_2d = euler2rot_2dinv(x[:, [0]]) # scale_2d = torch.exp(-self.trunc(x[:, :2]))
            trans_2d = x[:, 1:] # trans_2d = x[:, -2:]
            x_other = torch.bmm(rot_2d, (x_ori - trans_2d)[...,None]).squeeze(-1) # x_other =  (x_ori - trans_2d) * scale_2d
            if form == 0:
                if mode == 0:
    ...
    def inverse(self, deformation_code, input_pts, alpha_ratio):
        batch_size = input_pts.shape[0]
        # deform_code = deformation_code.repeat(batch_size,1)
        x = input_pts
        for i_b in range(self.n_blocks):
            ...
            rot_2d = euler2rot_2d(x[:, [0]]) # scale_2d = torch.exp(self.trunc(x[:, :2]))
            trans_2d = x[:, 1:] # trans_2d = x[:, 2:]
            x_focus = torch.bmm(rot_2d, x_focus[...,None]).squeeze(-1) + trans_2d # x_focus = scale_2d * x_focus + trans_2d
            ...
            x_other = x_ori + x # x_other = scale * x_ori + trans
            if form == 0:
                if mode == 0:
                    x = torch.cat([x_focus_ori, x_other], dim=-1)
                elif mode == 1:
...
# Deform
class AmbientNetwork(nn.Module):
    def __init__(self,
                 d_feature,
                 d_in,
                 d_out,
                 ...
                 bias=0.5,
                 weight_norm=True):
        super(AmbientNetwork, self).__init__()
...
class SDFNetwork(nn.Module):
    def __init__(self,
        ...
        if multires_ambient > 0:
            embed_amb_fn, input_ch_2 = get_embedder(multires_ambient, input_dims=d_in_2)
    ...
    def forward(self, input_pts, ambient_coord, alpha_ratio):
        ...
        if self.embed_amb_fn is not None:
            # Anneal
            ambient_coord = self.embed_amb_fn(ambient_coord, alpha_ratio)
        inputs = torch.cat([input_pts, ambient_coord], dim=-1)
        x = inputs
        for l in range(0, self.num_layers - 1):
            lin = getattr(self, "lin" + str(l))
    ...
    # Anneal
    def sdf(self, x, ambient_coord, alpha_ratio):
        return self.forward(x, ambient_coord, alpha_ratio)[:, :1]

    def sdf_hidden_appearance(self, x, ambient_coord, alpha_ratio):
        return self.forward(x, ambient_coord, alpha_ratio)

    def gradient(self, x, ambient_coord, alpha_ratio):
        x.requires_grad_(True)
        y = self.sdf(x, ambient_coord, alpha_ratio)
